...
#Limpiar valores nulos
clean_df = ta.utils.dropna(df)

#Streamlit
st.title("EDA para datos financieros")

a = datetime.utcfromtimestamp(clean_df['Timestamp'].head(1)).strftime('%Y-%m-%d %H:%M:%S')
b = datetime.utcfromtimestamp(clean_df['Timestamp'].tail(1)).strftime('%Y-%m-%d %H:%M:%S')
...
st.write('Number of rows: {}, Number of columns: {}'.format(*clean_df.shape))

st.subheader('Price')
st.line_chart(df['Close'], width=max_width)

st.subheader(f'Return {return_value} periods')
st.area_chart(clean_df['y'], width=max_width)

st.subheader('Target Histogram')
bins = list(np.arange(-10, 10, 0.5))
hist_values, hist_indexes = np.histogram(clean_df['y'], bins=bins)
...
x_cols = [col for col in clean_df.columns if col not in ['Timestamp', 'y'] and col.startswith(ta_col_prefix)]
labels = [col for col in x_cols]
values = [np.corrcoef(clean_df[col], clean_df['y'])[0, 1] for col in x_cols]
